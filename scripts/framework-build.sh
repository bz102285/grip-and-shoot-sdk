...
FRAMEWORK_FILE_NAME=GripAndShoot.a
...
lipo -create build/Release-iphoneos/libGripAndShootSDK.a -output ${OUTPUT}/Framework/$FRAMEWORK_FILE_NAME
...
if [ -e scripts/public-headers.txt ]
  then
    ...
    cat scripts/public-headers.txt | while read line; do
      if [ -e "GripAndShootSDK/$line" ]
        then
          echo "Copied header $line"
          cp GripAndShootSDK/$line $HEADERS_PATH
      fi
    done
fi
